...
const Roles = ({isOpen}: {isOpen: boolean}) => {
    const router = useRouter()
    const { data, error } = useSWR<ArtistsAPIGoodResponse, Error>("/api/artists")
                ...
                if (!usedRoles.has(artist.role.url)) {
                    ...
                    return (
                        ...
                        <People artists={data.artists}
                                role={artist.role}
                                isVisible={router.query.role === artist.role.url} />
                        </>
                    )
                }
...
const People = ({artists, role, isVisible}: { artists: ArtistWithExtras[], role: Role, isVisible: boolean }) => {
